...
/*  TAREFAS PENDENTES
    C) Adiciona um elemento em uma dada posição “pos” (lista *l, elemento *e, int pos). Re-
    tornar o código (1 para sucesso e 0 para Falha caso posição inválida);
    D) Remove um elemento no início da Lista (lista *l);. Retornar o endereço do elemento
    NULL para elemento inválido (lista vazia);
    E) Remove um elemento no final da Lista (lista *l);. Retornar o endereço do elemento ou
    NULL para elemento inválido (lista vazia);
    F) Remove um elemento de uma dada posição “pos” (lista *l, int pos);. Retornar o endere -
    ço do elemento ou NULL para elemento inválido ou lista vazia;
    G) Buscar um elemento contido na Lista (lista *l, char [ ] mod) (por modelo), Retornar a po -
    sição do elemento;
    H) Informa a quantidade de elementos da Lista (lista *l). Retornar a quantidade de elemen-
    tos;
    I) Mostrar a lista de elementos (lista *l);
    J) Apaga elemento, recebe o endereço do elemento e apaga o registro do sistema;
*/
